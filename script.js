...
const ice_sounds = [1,2,3,4,5].map((i) => {
    const fn = "./sfx/ice" + i + ".mp3";
    const a = new Audio(fn);
    a.load();
    return a;
});

const fire_sound = new Audio("./sfx/inferno.mp3");
fire_sound.load();
...
let qr_links = ["./qr.svg", "./bio.webp", "./phone.png"];
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const thaw = document.querySelector("#thaw");
    ...
    make_notification("You are now accessing \"The Zeagle: A Compendium Of East Meck Fables\"");
    make_notification("Archive removed from freezer");
    ...
    ice.addEventListener("click", () => {
        ...
        ice.style.filter = "brightness(1.1)";
        setTimeout(() => {
            ice.style.filter = "";
        }, 50);

        if (freezeclicks >= 5) {
            thaw.style.display = "inline";
        }
    })
...
let alerts = ["<span class=room>700</span> Hall Clo$ed For Repair$",
              ...
              "Keeping Zeagle Website unthawed too long selfish?"];
...
const freeze = () => {
    ...
    document.querySelectorAll("*").forEach(e => {
        e.style.animationDuration = "100000000s";
    });
    const x = window.scrollX;
    const y = window.scrollY;
    window.onscroll = () => { window.scrollTo(x, y); };
    document.querySelector("#ice").style.display = "inline";
    document.querySelectorAll("html, .eye").forEach(e => {
        e.style.backgroundImage = "url(bgstatic.jpg)";
    });
    freezeclicks = 0;
}

let freeze_timeout = null;
...
const unfreeze = () => {
    ...
    setTimeout(() => {
        frozen = false;
        make_notification("Archive Thawed");
        make_notification("WARNING: Please do not keep unfrozen for too long, this is a permenant installation.");
        document.querySelectorAll("*").forEach(e => {
            e.style.animationDuration = "";
        });
        window.onscroll = () => {
            clearTimeout(freeze_timeout);
            freeze_timeout = setTimeout(() => {
                freeze();
            }, 20000);
        };
        document.querySelector("#ice").style.display = "none";
        document.querySelectorAll("html, .eye").forEach(e => {
            e.style.backgroundImage = "url('./bg.gif')";
        });
    }, 500);
}
